...
__global__ void markingTopKMasks(int L, int top_K, uint64_t* point_list_keys, uint32_t* point_list, bool* masks)
{
	auto idx = cg::this_grid().thread_rank();
	if (idx >= L)
		return;
	...
	{
		uint32_t prevtile = point_list_keys[idx - top_K] >> 32;
		if (currtile != prevtile)
			masks[val] = true;
	}
}
...
int CudaRasterizer::Rasterizer::forwardTopKdT(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	bool debug)
{
	...
	// Let each tile blend its range of Gaussians independently in parallel
	const float* feature_ptr = colors_precomp != nullptr ? colors_precomp : geomState.rgb;
	CHECK_CUDA(FORWARD::topk_dt_gaussian(
		tile_grid, block,
		imgState.ranges,
		binningState.point_list,
		width, height,
