	...
	const int top_K_dT)
{
  ...
  // top_K_dT_mask
  torch::Tensor top_K_dT_mask = torch::full({P}, false, bool_opts);
  ...
  if(P != 0)
  {
	  ...
	  // overloaded forward, top_K_dT_mask
	  rendered = CudaRasterizer::Rasterizer::forwardTopKdT(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
		...
		top_K_dT,
		out_color.contiguous().data<float>(),
		top_K_dT_mask.contiguous().data<bool>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer, top_K_dT_mask);
}