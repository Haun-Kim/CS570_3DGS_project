  ...
  for i in "${(@i)prune_percents}"; do
    ...
    for prune_type in "${prune_types[@]}"; do
      ...
      while true; do
        gpu_id=0
        if [[ -n $gpu_id ]]; then
          echo "GPU $gpu_id is available. Starting train.py with dataset '$arg', prune_percent '$i', prune_type 'rad_splat' on port $port"
          CUDA_VISIBLE_DEVICES=$gpu_id nohup python train.py \
            -s "path_to_dataset/$arg" \
            -m "output/radsplat/360v2/${arg}_${i}" \
            --prune_method "rad_splat" \
            --prune_percent $i \
            --eval \
            --wandb_run_name "${arg}_${i}"\
            --port $port > "logs/radsplat/360v2/train_${arg}_${i}.log" 2>&1
