  ...
  for i in "${(@i)prune_percents}"; do
    ...
    prune_decay="${prune_decays: i}"
      ...
      while true; do
        gpu_id=1
        if [[ -n $gpu_id ]]; then
          echo "GPU $gpu_id is available. Starting train.py lp-3dgs with dataset '$arg', prune_type 'rad_splat', prune_decay on port $port"
          CUDA_VISIBLE_DEVICES=$gpu_id python train.py \
            -s "path_to_dataset/$arg" \
            -m "output/radsplat/${arg}_mask" \
            --prune_method "rad_splat" \
            --eval \
            --use_importance_mask \
            --wandb_run_name "$arg"\
            --port $port > "logs/radsplat/train_${arg}_mask.log" 2>&1 
